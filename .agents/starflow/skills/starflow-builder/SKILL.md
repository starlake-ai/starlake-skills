...
3. **Scaffold and fill.** Create `{starflow-root}/skills/starflow-<slug>/` from the chosen template(s). Every step file keeps the conventions: Rules with a Scale block, Preconditions on `stepsCompleted`, explicit `**HALT**` checkpoints with `Default:` lines, Save, and Next. (A workflow whose SKILL.md declares unattended mode inapplicable — like `starflow-retrospective` — omits the `Default:` lines; workflows without a persisted `stepsCompleted` file — like `starflow-code-review` — phrase Preconditions on their bound runtime variables instead.) If the workflow writes a document, also scaffold an output template in `{starflow-root}/templates/`.
